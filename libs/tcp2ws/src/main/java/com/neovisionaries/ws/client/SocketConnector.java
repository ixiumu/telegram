...
class SocketConnector
{
    ...
    SocketConnector(SocketFactory socketFactory, Address address, int timeout, String[] serverNames, int socketTimeout, String cdn)
    {
        this(socketFactory, address, timeout, socketTimeout, serverNames, null, null, null, 0, cdn);
    }
    ...
    SocketConnector(
            ...
            String host, int port, String cdn)
    {
    ...
    private InetAddress[] resolveHostname() throws WebSocketException
    {
        ...
        try
        {
            if (mCdn != null && !mCdn.isEmpty()) {
                // Resolve cdn to IP addresses.
                addresses = InetAddress.getAllByName(mCdn);
            } else {
                // Resolve hostname to IP addresses.
    ...
    private void doConnect() throws WebSocketException
    {
        ...
        if (mSocket instanceof SSLSocket)
        {
            // Set SNI
            try {
                String sniHost = (mHost != null) ? mHost : mAddress.getHostname();

                if (sniHost != null && !sniHost.isEmpty()) {
                    javax.net.ssl.SSLParameters params = ((SSLSocket) mSocket).getSSLParameters();
                    params.setServerNames(List.of(new SNIHostName(sniHost)));
                    ((SSLSocket) mSocket).setSSLParameters(params);
                }
            } catch (Exception e) {
                throw new WebSocketException(WebSocketError.SOCKET_CONNECT_ERROR, "Failed to manually set SNI.", e);
            }
